...
void ppVertex(Vertex p, int _i_)
{
  ...
  case is_VName:
    ...
    ppListAttr(p->u.vName_.listattr_, 0);
    renderC('>');
    if (_i_ > 0) renderC(_R_PAREN);
    break;
...
void shAttrVal(AttrVal p)
{
  ...
  default:
    fprintf(stderr, "Error: bad kind field when showing AttrVal!\n");
    exit(1);
  }
}
...
void shAttrName(AttrName p)
{
  ...
  default:
    fprintf(stderr, "Error: bad kind field when showing AttrName!\n");
    exit(1);
  }
}
...
void shListAttr(ListAttr p)
{
  ...
  default:
    fprintf(stderr, "Error: bad kind field when showing ListAttr!\n");
    exit(1);
  }
}
...
void shVertex(Vertex p)
{
  ...
  bufAppendC(' ');
    shListAttr(p->u.vName_.listattr_);
