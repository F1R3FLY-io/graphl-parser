/* File generated by the BNF Converter (bnfc 2.9.6). */

/* C Abstract Syntax Implementation. */

#include <stdio.h>
#include <stdlib.h>
#include "Absyn.h"

/********************   GNil    ********************/

Graph make_GNil()
{
    Graph tmp = (Graph) malloc(sizeof(*tmp));
    if (!tmp)
    {
        fprintf(stderr, "Error: out of memory when allocating GNil!\n");
        exit(1);
    }
    tmp->kind = is_GNil;
    return tmp;
}

/********************   GVertex    ********************/

Graph make_GVertex(Vertex p1, Graph p2)
{
    Graph tmp = (Graph) malloc(sizeof(*tmp));
    if (!tmp)
    {
        fprintf(stderr, "Error: out of memory when allocating GVertex!\n");
        exit(1);
    }
    tmp->kind = is_GVertex;
    tmp->u.gVertex_.vertex_ = p1;
    tmp->u.gVertex_.graph_ = p2;
    return tmp;
}

/********************   GVar    ********************/

Graph make_GVar(LVar p1, Graph p2)
{
    Graph tmp = (Graph) malloc(sizeof(*tmp));
    if (!tmp)
    {
        fprintf(stderr, "Error: out of memory when allocating GVar!\n");
        exit(1);
    }
    tmp->kind = is_GVar;
    tmp->u.gVar_.lvar_ = p1;
    tmp->u.gVar_.graph_ = p2;
    return tmp;
}

/********************   GNominate    ********************/

Graph make_GNominate(Binding p1)
{
    Graph tmp = (Graph) malloc(sizeof(*tmp));
    if (!tmp)
    {
        fprintf(stderr, "Error: out of memory when allocating GNominate!\n");
        exit(1);
    }
    tmp->kind = is_GNominate;
    tmp->u.gNominate_.binding_ = p1;
    return tmp;
}

/********************   GEdgeAnon    ********************/

Graph make_GEdgeAnon(Binding p1, Binding p2)
{
    Graph tmp = (Graph) malloc(sizeof(*tmp));
    if (!tmp)
    {
        fprintf(stderr, "Error: out of memory when allocating GEdgeAnon!\n");
        exit(1);
    }
    tmp->kind = is_GEdgeAnon;
    tmp->u.gEdgeAnon_.binding_1 = p1;
    tmp->u.gEdgeAnon_.binding_2 = p2;
    return tmp;
}

/********************   GEdgeNamed    ********************/

Graph make_GEdgeNamed(Name p1, Binding p2, Binding p3)
{
    Graph tmp = (Graph) malloc(sizeof(*tmp));
    if (!tmp)
    {
        fprintf(stderr, "Error: out of memory when allocating GEdgeNamed!\n");
        exit(1);
    }
    tmp->kind = is_GEdgeNamed;
    tmp->u.gEdgeNamed_.name_ = p1;
    tmp->u.gEdgeNamed_.binding_1 = p2;
    tmp->u.gEdgeNamed_.binding_2 = p3;
    return tmp;
}

/********************   GRuleAnon    ********************/

Graph make_GRuleAnon(Graph p1, Graph p2)
{
    Graph tmp = (Graph) malloc(sizeof(*tmp));
    if (!tmp)
    {
        fprintf(stderr, "Error: out of memory when allocating GRuleAnon!\n");
        exit(1);
    }
    tmp->kind = is_GRuleAnon;
    tmp->u.gRuleAnon_.graph_1 = p1;
    tmp->u.gRuleAnon_.graph_2 = p2;
    return tmp;
}

/********************   GRuleNamed    ********************/

Graph make_GRuleNamed(Name p1, Graph p2, Graph p3)
{
    Graph tmp = (Graph) malloc(sizeof(*tmp));
    if (!tmp)
    {
        fprintf(stderr, "Error: out of memory when allocating GRuleNamed!\n");
        exit(1);
    }
    tmp->kind = is_GRuleNamed;
    tmp->u.gRuleNamed_.name_ = p1;
    tmp->u.gRuleNamed_.graph_1 = p2;
    tmp->u.gRuleNamed_.graph_2 = p3;
    return tmp;
}

/********************   GSubgraph    ********************/

Graph make_GSubgraph(GraphBinding p1)
{
    Graph tmp = (Graph) malloc(sizeof(*tmp));
    if (!tmp)
    {
        fprintf(stderr, "Error: out of memory when allocating GSubgraph!\n");
        exit(1);
    }
    tmp->kind = is_GSubgraph;
    tmp->u.gSubgraph_.graphbinding_ = p1;
    return tmp;
}

/********************   GTensor    ********************/

Graph make_GTensor(Graph p1, Graph p2)
{
    Graph tmp = (Graph) malloc(sizeof(*tmp));
    if (!tmp)
    {
        fprintf(stderr, "Error: out of memory when allocating GTensor!\n");
        exit(1);
    }
    tmp->kind = is_GTensor;
    tmp->u.gTensor_.graph_1 = p1;
    tmp->u.gTensor_.graph_2 = p2;
    return tmp;
}

/********************   VBind    ********************/

Binding make_VBind(LVar p1, Vertex p2, Graph p3)
{
    Binding tmp = (Binding) malloc(sizeof(*tmp));
    if (!tmp)
    {
        fprintf(stderr, "Error: out of memory when allocating VBind!\n");
        exit(1);
    }
    tmp->kind = is_VBind;
    tmp->u.vBind_.lvar_ = p1;
    tmp->u.vBind_.vertex_ = p2;
    tmp->u.vBind_.graph_ = p3;
    return tmp;
}

/********************   GBind    ********************/

GraphBinding make_GBind(UVar p1, Graph p2, Graph p3)
{
    GraphBinding tmp = (GraphBinding) malloc(sizeof(*tmp));
    if (!tmp)
    {
        fprintf(stderr, "Error: out of memory when allocating GBind!\n");
        exit(1);
    }
    tmp->kind = is_GBind;
    tmp->u.gBind_.uvar_ = p1;
    tmp->u.gBind_.graph_1 = p2;
    tmp->u.gBind_.graph_2 = p3;
    return tmp;
}

/********************   AttributeValue    ********************/

AttrVal make_AttributeValue(LVar p1)
{
    AttrVal tmp = (AttrVal) malloc(sizeof(*tmp));
    if (!tmp)
    {
        fprintf(stderr, "Error: out of memory when allocating AttributeValue!\n");
        exit(1);
    }
    tmp->kind = is_AttributeValue;
    tmp->u.attributeValue_.lvar_ = p1;
    return tmp;
}

/********************   AttributeName    ********************/

AttrName make_AttributeName(LVar p1)
{
    AttrName tmp = (AttrName) malloc(sizeof(*tmp));
    if (!tmp)
    {
        fprintf(stderr, "Error: out of memory when allocating AttributeName!\n");
        exit(1);
    }
    tmp->kind = is_AttributeName;
    tmp->u.attributeName_.lvar_ = p1;
    return tmp;
}

/********************   AttributePair    ********************/

Attr make_AttributePair(AttrName p1, AttrVal p2)
{
    Attr tmp = (Attr) malloc(sizeof(*tmp));
    if (!tmp)
    {
        fprintf(stderr, "Error: out of memory when allocating AttributePair!\n");
        exit(1);
    }
    tmp->kind = is_AttributePair;
    tmp->u.attributePair_.attrname_ = p1;
    tmp->u.attributePair_.attrval_ = p2;
    return tmp;
}

/********************   EmptyAttrList    ********************/

ListAttr make_EmptyAttrList()
{
    ListAttr tmp = (ListAttr) malloc(sizeof(*tmp));
    if (!tmp)
    {
        fprintf(stderr, "Error: out of memory when allocating EmptyAttrList!\n");
        exit(1);
    }
    tmp->kind = is_EmptyAttrList;
    return tmp;
}

/********************   AttrList    ********************/

ListAttr make_AttrList(Attr p1, ListAttr p2)
{
    ListAttr tmp = (ListAttr) malloc(sizeof(*tmp));
    if (!tmp)
    {
        fprintf(stderr, "Error: out of memory when allocating AttrList!\n");
        exit(1);
    }
    tmp->kind = is_AttrList;
    tmp->u.attrList_.attr_ = p1;
    tmp->u.attrList_.listattr_ = p2;
    return tmp;
}

/********************   VName    ********************/

Vertex make_VName(Name p1, ListAttr p2)
{
    Vertex tmp = (Vertex) malloc(sizeof(*tmp));
    if (!tmp)
    {
        fprintf(stderr, "Error: out of memory when allocating VName!\n");
        exit(1);
    }
    tmp->kind = is_VName;
    tmp->u.vName_.name_ = p1;
    tmp->u.vName_.listattr_ = p2;
    return tmp;
}

/********************   NameWildcard    ********************/

Name make_NameWildcard()
{
    Name tmp = (Name) malloc(sizeof(*tmp));
    if (!tmp)
    {
        fprintf(stderr, "Error: out of memory when allocating NameWildcard!\n");
        exit(1);
    }
    tmp->kind = is_NameWildcard;
    return tmp;
}

/********************   NameVVar    ********************/

Name make_NameVVar(LVar p1)
{
    Name tmp = (Name) malloc(sizeof(*tmp));
    if (!tmp)
    {
        fprintf(stderr, "Error: out of memory when allocating NameVVar!\n");
        exit(1);
    }
    tmp->kind = is_NameVVar;
    tmp->u.nameVVar_.lvar_ = p1;
    return tmp;
}

/********************   NameGVar    ********************/

Name make_NameGVar(UVar p1)
{
    Name tmp = (Name) malloc(sizeof(*tmp));
    if (!tmp)
    {
        fprintf(stderr, "Error: out of memory when allocating NameGVar!\n");
        exit(1);
    }
    tmp->kind = is_NameGVar;
    tmp->u.nameGVar_.uvar_ = p1;
    return tmp;
}

/********************   NameQuoteGraph    ********************/

Name make_NameQuoteGraph(Graph p1)
{
    Name tmp = (Name) malloc(sizeof(*tmp));
    if (!tmp)
    {
        fprintf(stderr, "Error: out of memory when allocating NameQuoteGraph!\n");
        exit(1);
    }
    tmp->kind = is_NameQuoteGraph;
    tmp->u.nameQuoteGraph_.graph_ = p1;
    return tmp;
}

/********************   NameQuoteVertex    ********************/

Name make_NameQuoteVertex(Vertex p1)
{
    Name tmp = (Name) malloc(sizeof(*tmp));
    if (!tmp)
    {
        fprintf(stderr, "Error: out of memory when allocating NameQuoteVertex!\n");
        exit(1);
    }
    tmp->kind = is_NameQuoteVertex;
    tmp->u.nameQuoteVertex_.vertex_ = p1;
    return tmp;
}

/********************   ListName    ********************/

ListName make_ListName(Name p1, ListName p2)
{
    ListName tmp = (ListName) malloc(sizeof(*tmp));
    if (!tmp)
    {
        fprintf(stderr, "Error: out of memory when allocating ListName!\n");
        exit(1);
    }
    tmp->name_ = p1;
    tmp->listname_ = p2;
    return tmp;
}

/***************************   Cloning   ******************************/

Graph clone_Graph(Graph p)
{
  switch(p->kind)
  {
  case is_GNil:
    return make_GNil ();

  case is_GVertex:
    return make_GVertex
      ( clone_Vertex(p->u.gVertex_.vertex_)
      , clone_Graph(p->u.gVertex_.graph_)
      );

  case is_GVar:
    return make_GVar
      ( strdup(p->u.gVar_.lvar_)
      , clone_Graph(p->u.gVar_.graph_)
      );

  case is_GNominate:
    return make_GNominate (clone_Binding(p->u.gNominate_.binding_));

  case is_GEdgeAnon:
    return make_GEdgeAnon
      ( clone_Binding(p->u.gEdgeAnon_.binding_1)
      , clone_Binding(p->u.gEdgeAnon_.binding_2)
      );

  case is_GEdgeNamed:
    return make_GEdgeNamed
      ( clone_Name(p->u.gEdgeNamed_.name_)
      , clone_Binding(p->u.gEdgeNamed_.binding_1)
      , clone_Binding(p->u.gEdgeNamed_.binding_2)
      );

  case is_GRuleAnon:
    return make_GRuleAnon
      ( clone_Graph(p->u.gRuleAnon_.graph_1)
      , clone_Graph(p->u.gRuleAnon_.graph_2)
      );

  case is_GRuleNamed:
    return make_GRuleNamed
      ( clone_Name(p->u.gRuleNamed_.name_)
      , clone_Graph(p->u.gRuleNamed_.graph_1)
      , clone_Graph(p->u.gRuleNamed_.graph_2)
      );

  case is_GSubgraph:
    return make_GSubgraph (clone_GraphBinding(p->u.gSubgraph_.graphbinding_));

  case is_GTensor:
    return make_GTensor
      ( clone_Graph(p->u.gTensor_.graph_1)
      , clone_Graph(p->u.gTensor_.graph_2)
      );

  default:
    fprintf(stderr, "Error: bad kind field when cloning Graph!\n");
    exit(1);
  }
}

Binding clone_Binding(Binding p)
{
  switch(p->kind)
  {
  case is_VBind:
    return make_VBind
      ( strdup(p->u.vBind_.lvar_)
      , clone_Vertex(p->u.vBind_.vertex_)
      , clone_Graph(p->u.vBind_.graph_)
      );

  default:
    fprintf(stderr, "Error: bad kind field when cloning Binding!\n");
    exit(1);
  }
}

GraphBinding clone_GraphBinding(GraphBinding p)
{
  switch(p->kind)
  {
  case is_GBind:
    return make_GBind
      ( strdup(p->u.gBind_.uvar_)
      , clone_Graph(p->u.gBind_.graph_1)
      , clone_Graph(p->u.gBind_.graph_2)
      );

  default:
    fprintf(stderr, "Error: bad kind field when cloning GraphBinding!\n");
    exit(1);
  }
}

AttrVal clone_AttrVal(AttrVal p)
{
  switch(p->kind)
  {
  case is_AttributeValue:
    return make_AttributeValue (strdup(p->u.attributeValue_.lvar_));

  default:
    fprintf(stderr, "Error: bad kind field when cloning AttrVal!\n");
    exit(1);
  }
}

AttrName clone_AttrName(AttrName p)
{
  switch(p->kind)
  {
  case is_AttributeName:
    return make_AttributeName (strdup(p->u.attributeName_.lvar_));

  default:
    fprintf(stderr, "Error: bad kind field when cloning AttrName!\n");
    exit(1);
  }
}

Attr clone_Attr(Attr p)
{
  switch(p->kind)
  {
  case is_AttributePair:
    return make_AttributePair
      ( clone_AttrName(p->u.attributePair_.attrname_)
      , clone_AttrVal(p->u.attributePair_.attrval_)
      );

  default:
    fprintf(stderr, "Error: bad kind field when cloning Attr!\n");
    exit(1);
  }
}

ListAttr clone_ListAttr(ListAttr p)
{
  switch(p->kind)
  {
  case is_EmptyAttrList:
    return make_EmptyAttrList ();

  case is_AttrList:
    return make_AttrList
      ( clone_Attr(p->u.attrList_.attr_)
      , clone_ListAttr(p->u.attrList_.listattr_)
      );

  default:
    fprintf(stderr, "Error: bad kind field when cloning ListAttr!\n");
    exit(1);
  }
}

Vertex clone_Vertex(Vertex p)
{
  switch(p->kind)
  {
  case is_VName:
    return make_VName
      ( clone_Name(p->u.vName_.name_)
      , clone_ListAttr(p->u.vName_.listattr_)
      );

  default:
    fprintf(stderr, "Error: bad kind field when cloning Vertex!\n");
    exit(1);
  }
}

Name clone_Name(Name p)
{
  switch(p->kind)
  {
  case is_NameWildcard:
    return make_NameWildcard ();

  case is_NameVVar:
    return make_NameVVar (strdup(p->u.nameVVar_.lvar_));

  case is_NameGVar:
    return make_NameGVar (strdup(p->u.nameGVar_.uvar_));

  case is_NameQuoteGraph:
    return make_NameQuoteGraph (clone_Graph(p->u.nameQuoteGraph_.graph_));

  case is_NameQuoteVertex:
    return make_NameQuoteVertex (clone_Vertex(p->u.nameQuoteVertex_.vertex_));

  default:
    fprintf(stderr, "Error: bad kind field when cloning Name!\n");
    exit(1);
  }
}

ListName clone_ListName(ListName listname)
{
  if (listname)
  {
    /* clone of non-empty list */
    return make_ListName
      ( clone_Name(listname->name_)
      , clone_ListName(listname->listname_)
      );
  }
  else return NULL; /* clone of empty list */
}

/********************   Recursive Destructors    **********************/

/* These free an entire abstract syntax tree
 * including all subtrees and strings.
 *
 * Will not work properly if there is sharing in the tree,
 * i.e., when some pointers are aliased.  In this case
 * it will attempt to free the same memory twice.
 */

void free_Graph(Graph p)
{
  switch(p->kind)
  {
  case is_GNil:
    break;

  case is_GVertex:
    free_Vertex(p->u.gVertex_.vertex_);
    free_Graph(p->u.gVertex_.graph_);
    break;

  case is_GVar:
    free(p->u.gVar_.lvar_);
    free_Graph(p->u.gVar_.graph_);
    break;

  case is_GNominate:
    free_Binding(p->u.gNominate_.binding_);
    break;

  case is_GEdgeAnon:
    free_Binding(p->u.gEdgeAnon_.binding_1);
    free_Binding(p->u.gEdgeAnon_.binding_2);
    break;

  case is_GEdgeNamed:
    free_Name(p->u.gEdgeNamed_.name_);
    free_Binding(p->u.gEdgeNamed_.binding_1);
    free_Binding(p->u.gEdgeNamed_.binding_2);
    break;

  case is_GRuleAnon:
    free_Graph(p->u.gRuleAnon_.graph_1);
    free_Graph(p->u.gRuleAnon_.graph_2);
    break;

  case is_GRuleNamed:
    free_Name(p->u.gRuleNamed_.name_);
    free_Graph(p->u.gRuleNamed_.graph_1);
    free_Graph(p->u.gRuleNamed_.graph_2);
    break;

  case is_GSubgraph:
    free_GraphBinding(p->u.gSubgraph_.graphbinding_);
    break;

  case is_GTensor:
    free_Graph(p->u.gTensor_.graph_1);
    free_Graph(p->u.gTensor_.graph_2);
    break;

  default:
    fprintf(stderr, "Error: bad kind field when freeing Graph!\n");
    exit(1);
  }
  free(p);
}

void free_Binding(Binding p)
{
  switch(p->kind)
  {
  case is_VBind:
    free(p->u.vBind_.lvar_);
    free_Vertex(p->u.vBind_.vertex_);
    free_Graph(p->u.vBind_.graph_);
    break;

  default:
    fprintf(stderr, "Error: bad kind field when freeing Binding!\n");
    exit(1);
  }
  free(p);
}

void free_GraphBinding(GraphBinding p)
{
  switch(p->kind)
  {
  case is_GBind:
    free(p->u.gBind_.uvar_);
    free_Graph(p->u.gBind_.graph_1);
    free_Graph(p->u.gBind_.graph_2);
    break;

  default:
    fprintf(stderr, "Error: bad kind field when freeing GraphBinding!\n");
    exit(1);
  }
  free(p);
}

void free_AttrVal(AttrVal p)
{
  switch(p->kind)
  {
  case is_AttributeValue:
    free(p->u.attributeValue_.lvar_);
    break;

  default:
    fprintf(stderr, "Error: bad kind field when freeing AttrVal!\n");
    exit(1);
  }
  free(p);
}

void free_AttrName(AttrName p)
{
  switch(p->kind)
  {
  case is_AttributeName:
    free(p->u.attributeName_.lvar_);
    break;

  default:
    fprintf(stderr, "Error: bad kind field when freeing AttrName!\n");
    exit(1);
  }
  free(p);
}

void free_Attr(Attr p)
{
  switch(p->kind)
  {
  case is_AttributePair:
    free_AttrName(p->u.attributePair_.attrname_);
    free_AttrVal(p->u.attributePair_.attrval_);
    break;

  default:
    fprintf(stderr, "Error: bad kind field when freeing Attr!\n");
    exit(1);
  }
  free(p);
}

void free_ListAttr(ListAttr p)
{
  switch(p->kind)
  {
  case is_EmptyAttrList:
    break;

  case is_AttrList:
    free_Attr(p->u.attrList_.attr_);
    free_ListAttr(p->u.attrList_.listattr_);
    break;

  default:
    fprintf(stderr, "Error: bad kind field when freeing ListAttr!\n");
    exit(1);
  }
  free(p);
}

void free_Vertex(Vertex p)
{
  switch(p->kind)
  {
  case is_VName:
    free_Name(p->u.vName_.name_);
    free_ListAttr(p->u.vName_.listattr_);
    break;

  default:
    fprintf(stderr, "Error: bad kind field when freeing Vertex!\n");
    exit(1);
  }
  free(p);
}

void free_Name(Name p)
{
  switch(p->kind)
  {
  case is_NameWildcard:
    break;

  case is_NameVVar:
    free(p->u.nameVVar_.lvar_);
    break;

  case is_NameGVar:
    free(p->u.nameGVar_.uvar_);
    break;

  case is_NameQuoteGraph:
    free_Graph(p->u.nameQuoteGraph_.graph_);
    break;

  case is_NameQuoteVertex:
    free_Vertex(p->u.nameQuoteVertex_.vertex_);
    break;

  default:
    fprintf(stderr, "Error: bad kind field when freeing Name!\n");
    exit(1);
  }
  free(p);
}

void free_ListName(ListName listname)
{
  if (listname)
  {
    free_Name(listname->name_);
    free_ListName(listname->listname_);
    free(listname);
  }
}

