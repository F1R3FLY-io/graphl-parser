...
impl<'graph, 'visitor> Walker<'graph, 'visitor, (String, String)> {
    ...
    fn new(graph: &'graph Graph, visitor: &'visitor impl Visitor<(String, String)>) -> Self {
        Self {
            graph,
            visitor,
            stack: vec![graph],
...
mod tests {
    ...
    impl Visitor<OpenClosePair> for MyVisitor {
        ...
        fn visit_vertex(&self, vertex: &crate::ast::Vertex) -> OpenClosePair {
            (
                format!(
                    "<vertex {}>\n",
                    match &vertex.name {
        ...
        fn visit_nominate(&self, name: &str, vertex: &crate::ast::Vertex) -> OpenClosePair {
            (
                format!(
        ...
        fn visit_context(&self, name: &crate::ast::Name, context: &str) -> OpenClosePair {
            (
                format!(
...
"#
    ...
    fn test_vertext_context() {
        let graph: Graph = unsafe { psGraph(c"context \"foo=bar\" for a in <a> | {0}".as_ptr()) }
            .try_into()
            .unwrap();
        let visitor = create_visitor();
        let mut walker = Walker::new(&graph, &visitor);
