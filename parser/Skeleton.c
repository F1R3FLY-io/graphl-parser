...
void visitGraph(Graph p, Visitor *visitor, void *context)
{
  ...
  case is_GTensor:
    ...
    break;
  case is_GContext:
    visitor->visitIsGContextCallback(p, context);
    visitString(p->u.gContext_.string_, visitor, context);
    visitName(p->u.gContext_.name_, visitor, context);
    visitGraph(p->u.gContext_.graph_, visitor, context);
    break;
  case is_GNominate:
